/**
 * DevTools Easter Egg - Mazen Albasyouny Portfolio
 */
(function() {
  'use strict';
  ...
  async function showOverlay() {
    ...
    // ── Real stack (Mazen's Backend stack) ──
    await type('Frontend', 'dte-heading');
    checkItem('HTML5 & CSS3'); checkItem('Tailwind CSS'); checkItem('Vanilla JavaScript (ES6+)');
    line('');
    await type('Backend', 'dte-heading');
    checkItem('PHP 8 & Laravel (MVC)'); checkItem('MySQL & PDO Architecture'); checkItem('RESTful APIs');
    line('');
    await type('Deployment', 'dte-heading');
    checkItem('Vercel & Render'); checkItem('Git & GitHub');
    line('');
...
╚═════╝ ╚══════╝  ╚═══╝`;
    ...
    console.log('%cGitHub:\n%cgithub.com/Baskootaa', dim, accent);
    console.log('%cLinkedIn:\n%clinkedin.com/in/mazen-albasyouny', dim, accent);
